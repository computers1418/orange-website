...
class MedicineSheet extends StatefulWidget {
  final Function(MedicineModel) onChanged;

  const MedicineSheet({super.key, required this.onChanged});

  @override
  State<MedicineSheet> createState() => _MedicineSheetState();
}

class _MedicineSheetState extends State<MedicineSheet> {
  ...
  void initState() {
    ...
    super.initState();
    problemController
        .getAllMedicineList({"doctorId": doctorController.selectDoctor == null
        ? ""
        ...
        builder: (controller) => Column(
          children: [
            ...
            const BottomsheetHeader(
                title1: "CHOOSE",
                title2: "MEDICINE",
                sub: "Choose medicine from the options below"),
            const SizedBox(
              height: 20,
            ),
            Stack(
              ...
              children: [
                Container(
                  ...
                  child: Column(
                    children: [
                      controller.medicineIsFetching.value
                          ? Column(
                              children: [
                                const SizedBox(
                                  height: 8,
                          ...
                          : controller.getMedicineList.isEmpty
                              ? SizedBox()
                              : ListView.builder(
                                  itemBuilder: (context, index) {
                                    MedicineModel model =
                                        controller.getMedicineList[index];
                                    return Padding(
                                      padding:
                                  ...
                                  primary: false,
                                  shrinkWrap: true,
                                  itemCount: controller.getMedicineList.length,
                                ),
                      Visibility(
                          visible: height > 600,
